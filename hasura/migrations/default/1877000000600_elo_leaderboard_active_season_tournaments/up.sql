-- Reapply _leaderboard_elo to include eligible tournament ELO rows for
-- an active named season, and to give an active season a true Last
-- Match value instead of hiding the secondary column.
--
-- Problem: named seasons only ever included player_elo rows where
-- season_id = the season's UUID. Tournament matches always write
-- season_id = NULL (they're season-independent by design), so any
-- tournament played during a named season was invisible to that
-- season's ELO, rank, matches, and win streak.
--
-- Fix, scoped to ACTIVE named seasons only (completed seasons keep
-- their existing final-minus-starting secondary_value and
-- regular-only value untouched):
--   * value  = regular season current ELO (5000 baseline if the
--     player has no regular-season row at all) + SUM of eligible
--     tournament player_elo.change inside the season's date window.
--     Never the tournament row's own global `current`.
--   * secondary_value (Last Match) = the change on the player's
--     single most recent eligible row (created_at desc, match_id
--     desc tie-break) -- regular-only when Exclude Tournaments is
--     on, regular-or-tournament when it's off.
--   * matches_played and rank follow the same eligibility.
--   * win_streak was audited and needs no change: it already
--     computes from matches directly (no season_id column there),
--     already respects the season's date window, and already
--     excludes tournaments only when the toggle asks it to.
--
-- Rolling windows (7/30 day, SUM(change)), Peak, and unbounded
-- Current are untouched -- all three require season_id IS NULL, so
-- none of them can ever reach the new active-season branches.
--
